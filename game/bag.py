...
class Bag:
    ...
    def _update(self, new_tile_counts: Mapping[Tile, int]) -> None:
        ...
        """
        tiles = self.__tile_bag
        for tile, count in new_tile_counts.items(): # It loops through the Mapping provided
            new_count = tiles.get(tile, 0) + count # If the tile already exists, it'll add the new count to it, otherwise it's set to 0
            if new_count > 0:
                tiles[tile] = new_count # Updates the bag with the new tile count
            elif tile in tiles: # Otherwise if the count is negative, it will remove that many items from the bag.
                del tiles[tile]
    ...
    def remove_tiles_from_bag(self, num_of_factories: int) -> list[Tile]:
        ...
        if num_of_factories not in (5, 7, 9):
            raise ValueError("You can only have 5, 7, or 9 factories! Please check the Azul rulebook for guidance.")
        
        tiles_to_remove: list[Tile] = []
        tiles_list: list[Tile] = [
            tile
            for tile in self.__tile_bag
        ...
        for _ in range(0, num_of_factories * 4): # For each Factory times 4...
            tiles_to_remove.append(tiles_list[randrange(1, len(tiles_list))]) # ...a random number is generated, and the tile at
                                                                              # that index is appended to the list.

        for tile in tiles_to_remove: # Then for each tile in that list, it's removed from the bag.
            self._remove(tile)
        
        return list(tiles_to_remove) # And the whole list is returned, in chunks of 4.
