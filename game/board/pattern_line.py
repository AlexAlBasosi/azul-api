...
class PatternLine:
    ...
    def place_tile_onto_pattern_line(self, tiles: list[Tile], tile_type: str, line_index: int) -> list[Tile]:
        """
        ...
        if not self._is_type_in_line(tile_type, self.__pattern_lines[line_index]):
            raise TypeError("Pattern Line contains a Tile type of a different colour. You can only place Tiles of the same colour onto the line.")
        ...
        if(tile_length is not None and space_remaining is not None):
            ...
            if(tile_length > space_remaining):
                raise OverflowError(f"Selected Pattern Line only has {space_remaining} space(s) remaining! Please try again but with less Tiles.")
